...
declare
	media_salarios real;
begin
	select avg(salario) into media_salarios
	from tbl_empregados;
...
create or replace function fc_salarios(cod_emp int)
returns table(salarios real, maisdez real, menosquinze real)
language plpgsql as
$$
declare
	salarios real;
	...
	menosquinze real;
begin
	select salario into salarios 
	from tbl_empregados 
	where cod_empregado = cod_emp; 
	
	maisdez := salarios * 1.10; 
	menosquinze := salarios * 0.85; 
	
	return query select salarios, maisdez, menosquinze;
end;
$$;

select fc_salarios(5);

-- Outra maneira de fazer, retornando 3 colunas no resultado
create or replace function fc_salarios2(p_cod_empregado int) 
returns table(salario_atual real, salario_acrescido real, salario_reduzido real)
language plpgsql as 
$$
begin
    return query 
...
--6 Crie a funcao fc_proj_departamento que recebe o codigo do departamento e mostra quantos projetos o departamento possui
create or replace function fc_proj_departamento(p_cod_departamento int) 
returns int 
language plpgsql as 
$$
declare
